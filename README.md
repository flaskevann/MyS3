...
For upload something like <code>aws s3 sync [path-to-local-folder] [path-to-s3-bucket]</code> works fine. For download you switch folder path with S3 bucket path and get <code>aws s3 sync [path-to-folder] [path-to-S3-bucket]</code>.
